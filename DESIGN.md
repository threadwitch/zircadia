...
**CLI tooling provenance (2026-08-01).** Nushell and jj now come from upstream-tracking repos rather than Fedora/Terra, which lag upstream. Nushell ships from the unsigned Gemfury repo (`gpgcheck=0`, RPM scriptlets disabled because `%post` assumes a mutable FHS and fails under composefs like 1Password; plugins register on first login). jj ships from the GPG-signed `aldantanneo/jj-vcs` COPR with weak deps disabled. Both are pinned to current upstream (Nushell 0.114.1, jj 0.43.0) for deliberate versioning, and both have Renovate `github-releases` annotations so updates are proposed, not auto-applied. The Gemfury unsigned-RPM supply-chain downgrade is accepted as the only way to track Nushell upstream via dnf; it is a documented debt until a signed upstream RPM or a hermetic build exists.
